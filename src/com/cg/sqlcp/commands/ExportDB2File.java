...
public class ExportDB2File {
	...
	public void start(String[] args) {
		...
		CLIRules rules = new CLIRules("copies the result of a SELECT to a file: "+SQLcp.class.getName()+" db2file ");
		...
		rules.addOptional(CLIParams.DEST_FILE_NAME, null).setDescription("Target: filename, if not given console will become target");
		//+++rules.addOptional(CLIParams.DEST_FILE_INCLHEADER, "NO").setDescription("YES=first line of destFile will contain columnnames");
		rules.addFlag(CLIParams.DEST_FILE_INCLHEADER).setDescription("YES=first line of destFile will contain columnnames");
		rules.addOptional(CLIParams.DEST_FILE_FIELDSEPARATOR, ";").setDescription("character to separate different values within the same line");
		rules.addOptional(CLIParams.DEST_FILE_DESTMODE, "OVERWRITE").setDescription("filemode for targetfile").setValidValues("OVERWRITE", "APPEND");
		//+++
		...
		rules.addOptional(CLIParams.DEST_FILE_FMT_BOOLFALSE, "FALSE").setDescription("output value for SQL BOOLEAN:FALSE");
		rules.addOptional(CLIParams.DEST_FILE_FMT_DATE, "+++").setDescription("output value for SQL DATE. see JAVA SimpleDateFormat");
		rules.addOptional(CLIParams.DEST_FILE_FMT_TIME, "+++").setDescription("output value for SQL TIME. see JAVA SimpleDateFormat");
		rules.addOptional(CLIParams.DEST_FILE_FMT_DATETIME, "+++").setDescription("foutput value for SQL DATETIME. see JAVA SimpleDateFormat");
		rules.addOptional(CLIParams.DEST_FILE_FMT_TIMESTAMP, "+++").setDescription("output value for SQL TIMESTAMP. see JAVA SimpleDateFormat");
		rules.addOptional(CLIParams.DEST_FILE_FMT_TIMESTAMPTZ, "+++").setDescription("output value for SQL TIMESTAMP WITH TIMEZONE. see JAVA SimpleDateFormat");
		rules.addOptional(CLIParams.DEST_FILE_FMT_CURRENCY, "+++").setDescription("output value for SQL CURRENCY. see JAVA SimpleDateFormat");
		rules.addOptional(CLIParams.DEST_FILE_FMT_FLOAT, "+++").setDescription("output value for SQL NUMERIC(and subtypes). see JAVA DecimalFormat");
		...
		try {
			...
			while (in.isAlive()) {
				...
				if (currMax>maxMemUsage) maxMemUsage = currMax;
			}
	...
	private void printSummary(ThreadReadingFromDB in ,CLIParsed cli) {
		...
		Util.log("time wait, db thread in-buffer full (max="+in.getMaxQueueSize()+"), waiting for writing thread(ms)="+in.getWaitForQueueConsumer());
	...
	private BufferedWriter prepareDestFile(boolean destOverwrite, boolean destAppend, String destfile) throws IOException {
	...
	private void writeRows(final Row[] rows, final ThreadReadingFromDB in, BufferedWriter w, String separator) 
		...
		for (int rCnt=0; rCnt<rows.length; rCnt++) {
			...
			for (int i=0; i<in.getColumnCount(); i++) {
				if (needsSeparator) b.append(separator);
